...
An SHA whose deploy invocation already failed is abandoned. The sole exception is a *configuration-only preflight refusal*: the invocation must stop at the initial production-config guard before a release directory is created or used, the failure must be exclusively missing or placeholder production configuration, and sanitized release-authority evidence must prove full artifact verification, no dependency install, application preflight, database mutation, activation, restart, health check, traffic, or public exposure. For a restored database, the original `API_KEY_PEPPER` must be present and unchanged; missing or changed pepper is NO-GO. Without explicit release-authority approval for one later canonical activation attempt, build and transfer a new merged SHA before using this procedure.
