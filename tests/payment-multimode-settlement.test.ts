...
function settle(kind: 'token_pack' | 'monetary_pack' | 'rolling_time' | 'payg') {
  ...
  const result = billing.settlePaymentSnapshot(account.id, paymentId, { method: kind, planId: kind === 'rolling_time' ? 'rolling' : 'pack', modelGroupId: 'value', tokens: kind === 'token_pack' ? 500000 : 0, balanceCents: kind === 'monetary_pack' || kind === 'payg' ? 1000 : 0 });
  return { ...result, db, account, billing, paymentId };
}
