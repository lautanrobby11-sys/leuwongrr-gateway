...
function openDatabase(): GatewayDatabase { const created=createTempDatabase(); dispose=created.dispose; return created.db; }
afterEach(()=>{ if(dispose){dispose();dispose=null;} });
...
describe('API key lifecycle',()=>{
  ...
  it('rejects unknown and malformed credentials',()=>{
    ...
    expect(db.authenticate('lwrr_live_notarealkeyvaluethatshouldneverauthenticate')).toBeNull();
...
describe('tenant budget isolation',()=>{
  it('scopes reservations and settlement per tenant',()=>{
    ...
    expect(db.db.prepare('SELECT units,state FROM usage_events WHERE id=?').get(reservation)).toEqual({units:60,state:'reserved'});
    db.settleBudget(reservation,'tenant-a',42);
    expect(db.db.prepare('SELECT units,state FROM usage_events WHERE id=?').get(reservation)).toEqual({units:42,state:'settled'});
  });
});
