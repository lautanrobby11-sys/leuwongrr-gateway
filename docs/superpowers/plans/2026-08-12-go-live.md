...
- A single owner holds full control over planning, design, source, tests, commits, artifacts, release, VPS, approval, monitoring, and Notion. There is no second actor.
...
- [ ] **Step 2: Record one checkpoint**

Owner updates Notion with status `IN_PROGRESS` and no DONE claim. If the Worker directory is not confirmed as Gateway-owned and safe, leave it uncommitted and do not package it.
...
Owner records `BLOCKED` for any missing external item and changes it to `VERIFIED` only after sanitized evidence is available. No credential values are recorded.
...
Record full SHA, artifact checksum, signer fingerprint, both lockfile installs, `ci:local` result, previous release SHA, migration ID, and rollback target. Owner mirrors only sanitized evidence to Notion.
...
Owner updates Notion to `DEPLOYED` only after the host evidence is complete. Local and GitHub must point to the same full SHA.
...
Owner writes the final Notion entry only after all evidence is supplied. Status is `GO-LIVE` only when no required blocker remains; otherwise it remains `BLOCKED` with the exact missing evidence.
