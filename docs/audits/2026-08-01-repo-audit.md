...
| A16 | MEDIUM | `scripts/deploy.sh` | Artefak hanya di-checksum, tidak ditandatangani. Berkas `.sha256` dibawa bersama tarball, jadi hanya mendeteksi korupsi, bukan pemalsuan. **DITUTUP 8 Agustus 2026.** Artefak kini ditandatangani operator dengan `ssh-keygen -Y sign` (Ed25519, kunci dedicated `release-signer`); `deploy.sh` menolak artefak tanpa `.sha256.sig` yang lolos verifikasi terhadap trust anchor host `/opt/leuwongrr-gateway/config/release-signers`. Tanda tangan deterministik (tidak melemahkan A14). Lihat `docs/adr/ADR-013-artifact-signing.md`. |
