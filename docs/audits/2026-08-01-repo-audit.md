...
| A14 | MEDIUM | `scripts/build-release.sh` | Artefak tidak reproducible: `RELEASE` memuat `built_at` dan tar tidak dinormalisasi (mtime/owner), sehingga SHA tarball berbeda tiap build dari commit yang sama. Komentar di berkas itu menyiratkan sebaliknya |
