...
const violations=[];
async function walk(dir){for(const entry of await readdir(dir,{withFileTypes:true})){if(ignored.has(entry.name))continue;const path=join(dir,entry.name);const rel=relative(process.cwd(),path);if(!canonicalExceptions.has(rel)&&(forbidden.test(entry.name)||entry.name==='docker-compose.override.yml'))violations.push(rel);if(entry.isDirectory())await walk(path);}}
